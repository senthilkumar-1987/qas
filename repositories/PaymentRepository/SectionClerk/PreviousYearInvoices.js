...
exports.GET_BAD_DEPT_INVOICES_PREVIOUS_YEAR = async (status) => {
    let currentYears = moment().year() - 1;
    // let currentYears = moment().subtract(1, 'years').endOf('week');
    console.log(currentYears)
    let fromdate = currentYears + '-01-01 00:00:00:000'
    let todate = currentYears + '-12-31 23:59:50:999'

    // let query = ` SELECT Invoice_no,sum(Sub_total_rm) totalAmount,sum(Sub_total) AS Sub_total,
    // payment_mode,currency,company_name,User_name,Invoice_date,File_no from
    //  tbl_sirim_invoice_master WHERE Invoice_date between  '${fromdate}' and '${todate}' and bad_debt_status in (${status})  AND status in (1,3)
    ...
    GROUP BY Invoice_No,payment_mode,currency,company_name,User_name, CAST(Invoice_date AS DATE) ,File_no,MasterInvoiceNo 
    UNION
    SELECT Sub_total,Sub_total_rm,Invoice_no,
     payment_mode,currency,company_name,User_name, CAST(Invoice_date AS DATE) AS Invoice_Date,File_no,MasterInvoiceNo
    FROM
