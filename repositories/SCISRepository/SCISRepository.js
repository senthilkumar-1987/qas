...
exports.getProductList = async (sessionObj, req, res) => {
    ...
    let query = "Select Distinct (ProdName),ProdId from tbl_lib_prod order by ProdName asc";
...
exports.getISSonQuatationRepo = async (sessionObj, req, res) => {
    ...
    let query = `select * from tbl_IssuedQuotation tio WHERE 
    ...
    resObj.ISSonQuationlist = await mainDb.executeQuery(query);
   ...
   from tbl_IssuedQuotation tio WHERE 
    ...
    resObj.ISSonQuationPercentageData = await mainDb.executeQuery(query);
...
exports.getAepBySectorReportRepo = async (sessionObj, req, res) => {
    ...
    a.SicId=c.SicId and u.SecId=t.SecId AND u.FullName='ABD RAHIM WASIK' AND t.SectorName='CHEMICAL & CONSUMER SECTOR'
...
exports.getQuotationReportListRepo = async (sessionObj, req, res) => {
    ...
    let Sectorname = req.body.sectorName;
    let licenseeType = req.body.licenseeType;
...
exports.getTihtenSurvellanceRepo = async (sessionObj, req, res) => {
    ...
    dbo.fnStandardByProductID(m.ProdId) as 'Standard',th.CPApproveDate,
    DateAdd(M,coalesce(th.TightenSurvPeriod,0),th.CPApproveDate) as 'End Date'
...
exports.getFileInfoPreviewReportRepo = async (sessionObj, req, res) => {
    ...
    let query = `IF OBJECT_ID('tempdb..#temp') IS NOT NULL DROP TABLE #temp
    ...
    dc.ProdName 'Product',
    dbo.fnStandardByProductID(m.prodid) 'Standard',cc.CertNo 'Licence No',cc.ExpiryDate 'Expiry Date'
    ,sc.SchemeName 'Scheme',fs.Description 'Status',case when f.LabelLicencing='1' then 'Yes' else 'No' end 'Licencing Programme',
    cc.StartDate,st.SectorName,lt.Code,crpt.SurveilFreq,f.FileStatus
    ...
    select * from ( select *, row_number() over(partition by [File No],PrevFileNo,[Company Name],Officer,Product,Standard,[Licence No],[Expiry Date],Scheme,Status,[Licencing Programme],StartDate,SectorName,code,SurveilFreq,FileStatus,TreasureCategory/*,RelatedAgency*/,PreCertAcceptDate order by SusEndDate desc) as rn from #temp) t
    where t.rn = 1
...
exports.getCPTerminationReportListRepo = async (sessionObj, req, res) => {
    ...
    let year = req.body.year;
    let dateofCP = req.body.dateofCP;
    let sector = req.body.sector;
    let po = req.body.po;
    let cpDecision = req.body.cpDecision;
...
exports.getAudiUtilReportListRepo = async (sessionObj, req, res) => {
    ...
    ,SUM(a.Surveillance) As Surveillance
    ,SUM(a.InitialAudit) as InitialAudit
    ...
    ,SUM(a.ForeignInsp) as ForeignInsp
...
exports.getAudiUtilICCSReportListRepo = async (sessionObj, req, res) => {
    ...
    let query = `SELECT UserId, FullName, SectorName, role, SectorCode, ProdTarget, ISNULL(NewLicence, 0) AS NewLicence, 
	ISNULL(LSurveillance, 0) As LSurveillance, ISNULL(FSurveillance, 0) As FSurveillance, ISNULL(LInitialAudit, 0) As LInitialAudit, 
	ISNULL(FInitialAudit, 0) As FInitialAudit, ISNULL(Verification, 0) AS Verification, ISNULL(ChangeRequest, 0) As ChangeRequest,
	ISNULL(LabelLicensing, 0) As LabelLicensing, 	
	(ISNULL(LSurveillance, 0) + ISNULL(FSurveillance, 0) + ISNULL(LInitialAudit, 0) + 
	...
	ISNULL(FInitialAudit, 0) + ISNULL(Verification, 0) + ISNULL(ChangeRequest, 0) + ISNULL(LabelLicensing, 0)),0) / ISNULL(ProdTarget,0)),0) * 100),2) As Numeric(36,2))
	END As AudUtiRate,		
	ISNULL(ProdTotAmount, 0) As ProdTotAmount, ISNULL(GETarget, 0) As GETarget, 
	ISNULL(Consignment, 0) As Consignment, ISNULL(GEConsignment, 0) As GEConsignment, ISNULL(GECertification, 0) As GECertification, 
	ISNULL(BatchCert, 0) As BatchCert, ISNULL(BatchVer, 0) AS BatchVer, ISNULL(geTotAmount, 0) As geTotAmount,
	ISNULL(EISTarget, 0) As EISTarget, ISNULL(EisCntInspec, 0) As EisCntInspec, ISNULL(EisTotAmount, 0) As EisTotAmount
From (
	--PROD CERT
	SELECT U.UserId, u.FullName, s.SectorName, UPPER(a.role) AS role, s.SectorCode, ut.Target As ProdTarget 
		,SUM(a.NewLicence) AS NewLicence
...
exports.getGEAppReportListRepo = async (sessionObj, req, res) => {
    ...
    let projectOfficeName = req.body.projectOfficer;
